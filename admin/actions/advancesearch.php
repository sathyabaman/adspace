...
<head>
    <meta charset="UTF-8"/>
 	<title>Filter Properties - adspace.lk</title>
    <meta name="Description" content="adspace.lk is the best way to sell and buy homes, houses, online in sri lanka" />  
    <meta name="Keywords" content="search, filter, properties, adspace, adspace.lk, real estate, search, sri lanka" />
...
<?php	
	  ...
	  $category =$_GET['cat'];
	  $propert_typ =$_GET['pro_type'];
	  $district =$_GET['distri'];
	  $val = $_GET['value'];
...
<?php 
	...
	$sql = "SELECT * FROM property WHERE category='".$category."' AND propert_typ='".$propert_typ."' AND district='".$district."' AND verified='2' ORDER BY id DESC ";
	$sql .= "LIMIT {$per_page} ";
    ...
    <ul>
			   <?php
                ...
                if($pagination->total_pages() > 1) {
                                        ...
                                        echo "<li><a href=\"advancesearch.php?page=".$pagination->previous_page()."&value=".$val."&cat=".$category."&pro_type=".$propert_typ."&distri=".$district."\">Back</a> </li>";
                                         }
                        ...
                        for($i=1; $i <= $pagination->total_pages(); $i++) {
                                    if($i == $page) {
                                        ...
                                        echo " <li><a href=\"advancesearch.php?page={$i}&value=".$val."&cat=".$category."&pro_type=".$propert_typ."&distri=".$district."\">{$i}</a></li>";
                                    }
                                        ...
                                        if($pagination->has_next_page()) { 
                                                ...
                                                echo "<li><a href=\"advancesearch.php?page=".$pagination->next_page()."&value=".$val."&cat=".$category."&pro_type=".$propert_typ."&distri=".$district."\">next</a> </li>";
