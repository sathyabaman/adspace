...
<head>
    <meta charset="UTF-8"/>
    <title>Land - adspace.lk</title>
    <meta name="Description" content="real estate, adspace.lk is the best way to sell and buy lands online in sri lanka" />  
    <meta name="Keywords" content="adspace, adspace.lk, real estate, lands, lands for sale, lands for lease, lands for rent, submit a free ad, sri lanka" />
...
<?php 
	...
	$per_page = 12;
	$sql_count="SELECT COUNT(category) FROM  property WHERE category='Land'";
	
	// counting total no of ads in category room...
	$total_count = property::count_for_pagination("Land");
	$pagination = new Pagination($page, $per_page, $total_count);
	
	
	$sql = "SELECT * FROM property WHERE verified=2 AND category='Land' ORDER BY id DESC ";
	$sql .= "LIMIT {$per_page} ";
...
<h1 class="page-header">Listing all Lands..  (<?php echo $total_count ;?>)</h1>
    ...
    <ul>
			   <?php
                ...
                if($pagination->total_pages() > 1) {
                                        ...
                                        echo "<li><a href=\"land.php?page=".$pagination->previous_page()."\">Back</a> </li>";
                                         }
                        ...
                        for($i=1; $i <= $pagination->total_pages(); $i++) {
                                    if($i == $page) {
                                        ...
                                        echo " <li><a href=\"land.php?page={$i}\">{$i}</a></li>";
                                    }
                                        ...
                                        if($pagination->has_next_page()) { 
                                                ...
                                                echo "<li><a href=\"land.php?page=".$pagination->next_page()."\">next</a> </li>";
