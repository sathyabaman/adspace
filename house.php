...
<head>
    <meta charset="UTF-8"/>
    <title>House - adspace.lk</title>
    <meta name="Description" content="real estate, adspace.lk is the best way to sell and buy homes, houses, online realestate in sri lanka" />  
    <meta name="Keywords" content="adspace, adspace.lk, real estate, home for rent, home for sale, home for lease, house for rent, house for sale, house for lease, submit a free ad, sri lanka" />
...
<?php 
	...
	$per_page = 15;
	$sql_count="SELECT COUNT(category) FROM  property WHERE category='House'";
	
	// counting total no of ads in category room...
	$total_count = property::count_for_pagination("House");
	$pagination = new Pagination($page, $per_page, $total_count);
	
	
	$sql = "SELECT * FROM property WHERE verified=2 AND category='House' ORDER BY id DESC ";
	$sql .= "LIMIT {$per_page} ";
...
?>
		...
		<h1 class="page-header">Listing all houses..  (<?php echo $total_count ;?>)</h1>
...
<div class="pagination pagination-centered">
   ...
   <?php
	...
	if($pagination->total_pages() > 1) {
							...
							echo "<li><a href=\"house.php?page=".$pagination->previous_page()."\">Back</a> </li>";
							 }
			...
			for($i=1; $i <= 14; $i++) {
						if($i == $page) {
							...
							echo " <li><a href=\"house.php?page={$i}\">{$i}</a></li>";
						}
			}
							...
							if($pagination->has_next_page()) { 
									...
									echo "<li><a href=\"house.php?page=".$pagination->next_page()."\">next</a> </li>";
