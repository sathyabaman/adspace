...
<head>
    <meta charset="UTF-8"/>
    <title>Rooms - adspace.lk</title>
    <meta name="Description" content="real estate, adspace.lk is the best way find a room for rent in sri lanka" />  
    <meta name="Keywords" content="adspace, adspace.lk, real estate, rooms, rooms for rent, colombo, submit a free ad, sri lanka" />
...
<?php 
	...
	$per_page = 12;
	$sql_count="SELECT COUNT(category) FROM  property WHERE category='Rooms'";
	
	// counting total no of ads in category room...
	$total_count = property::count_for_pagination("Rooms");
	$pagination = new Pagination($page, $per_page, $total_count);
	
	
	$sql = "SELECT * FROM property WHERE verified=2 AND category='Rooms' ORDER BY id DESC ";
	$sql .= "LIMIT {$per_page} ";
...
<h1 class="page-header">Listing all Rooms...  (<?php echo $total_count ;?>)</h1>
        ...
        <ul>
			   <?php
                ...
                if($pagination->total_pages() > 1) {
                                        ...
                                        echo "<li><a href=\"room.php?page=".$pagination->previous_page()."\">Back</a> </li>";
                                         }
                        ...
                        for($i=1; $i <= $pagination->total_pages(); $i++) {
                                    if($i == $page) {
                                        ...
                                        echo " <li><a href=\"room.php?page={$i}\">{$i}</a></li>";
                                    }
                                        ...
                                        if($pagination->has_next_page()) { 
                                                ...
                                                echo "<li><a href=\"room.php?page=".$pagination->next_page()."\">next</a> </li>";
